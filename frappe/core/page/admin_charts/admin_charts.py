...
def get_sales_pie_data(from_date=None,to_date=None):
	...
	if from_date and to_date:
		...
		stmt="select item_category,(sales_amount/total_sales_amount)*100 as sales_perc from(select i.item_category,sum(amount) as sales_amount,(select sum(amount) from `tabSales Invoice Item` sii_ inner join tabItem i_ on sii_.item_code=i_.item_code and sii_.docstatus=1 and  i_.item_category in(select distinct item_category from `tabItem`) and sii_.creation between coalesce(null,'1111-09-01 14:49:40') and coalesce(null,'9999-09-01 14:49:40') ) as total_sales_amount from `tabSales Invoice Item` sii inner join `tabItem` i on sii.item_code=i.item_code where sii.docstatus=1 and i.item_category in(select distinct item_category from `tabItem`) and sii.creation between coalesce(null,'1111-09-01 14:49:40') and coalesce(null,'9999-09-01 14:49:40') group by i.item_category,total_sales_amount)foo"	
