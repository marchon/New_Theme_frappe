...
frappe.ui.ProductCatlog = Class.extend({
	init: function(args) {
		$.extend(this, args);
		this.nodes = {};
		this.image_mapper = {}
		...
		this.rootnode = new frappe.ui.TreeNode({
			...
			parent_label: null,
			expandable: true,
			image_mapper: this.image_mapper,
			root: true,
			data: {
				value: this.label,
				expandable: true
...
frappe.ui.TreeNode = Class.extend({
	init: function(args) {
		...
		var me = this;
		if (args.data.image_url){
			// s[this.label] = args.data.image_url
			me.image_mapper[this.label] = args.data.image_url
		}	
